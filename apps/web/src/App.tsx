...
function workspaceViewFromHistory(value: unknown): WorkspaceView | null {
  return value === 'overview' || value === 'setup' || value === 'telegram' || value === 'sources' || value === 'access' ? value : null;
}
...
export function App() {
  ...
  useEffect(() => {
    ...
    async function checkSetup() {
      ...
      try {
        ...
        if (cancelled) return;
        if (!complete) { setActiveView('setup'); window.history.replaceState(historyStateWithView('setup'), '', window.location.href); }
      } catch {
        if (cancelled) return;
        setActiveView('setup'); window.history.replaceState(historyStateWithView('setup'), '', window.location.href);
  ...
  useEffect(() => {
    ...
    if (initialView) setActiveView(initialView); else window.history.replaceState(historyStateWithView('overview'), '', window.location.href);
    function handlePopState(event: PopStateEvent) {
      const view = workspaceViewFromHistory(event.state?.superSignalsView) ?? 'overview'; setActiveView(view); setMenuOpen(false); if (view === 'overview') void refreshSharedSources();
    }
  ...
  async function handleLogin(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const authenticatedAccount = await readJson<Account>(response); setAccount(authenticatedAccount); setAuthState('signed-in'); setActiveView('overview'); window.history.replaceState(historyStateWithView('overview'), '', window.location.href); event.currentTarget.reset();
  ...
  async function handleLogout() {
    ...
    finally { setAccount(null); setSharedSources([]); setSharedSourcesLoaded(false); setSetupCheckComplete(false); setAuthState('signed-out'); setNotice(null); setActiveView('overview'); setMenuOpen(false); window.history.replaceState({}, '', window.location.href); setBusy(false); }
  }
  ...
  function navigate(view: WorkspaceView) {
    if (view !== activeView) window.history.pushState(historyStateWithView(view), '', window.location.href);
    ...
    return <main className="app-shell workspace-shell"><section className="dashboard-card dashboard-card--workspace" aria-label="Super Signals workspace">
      ...
      <aside className={`workspace-drawer ${menuOpen ? 'workspace-drawer--open' : ''}`} aria-label="Workspace navigation" aria-hidden={!menuOpen}>
        ...
        <nav className="drawer-nav" aria-label="Main menu">
          ...
          <button type="button" aria-current={activeView === 'access' ? 'page' : undefined} onClick={() => navigate('access')}><span className="drawer-nav-icon" aria-hidden="true">05</span><span className="drawer-nav-label"><strong>Access &amp; security</strong><small>Role, permissions and account security</small></span></button>
        </nav>
      ...
      <div className="workspace-content">
        ...
        {activeView === 'overview' && <section aria-labelledby="dashboard-title">
          <div className="overview-hero"><p className="eyebrow">{account.role_label}</p><h1 id="dashboard-title">Welcome back, {displayName}</h1><p className="intro">This is your operational dashboard. Detailed connection, source and security settings now live behind the menu instead of filling the homepage.</p><div className="overview-actions">{account.role === 'trading_admin' && canManageTelegram && <button className="button" type="button" onClick={() => navigate('setup')}>Add my Telegram groups</button>}{account.role !== 'trading_admin' && canManageTelegram && <button className="button" type="button" onClick={() => navigate('sources')}>Add a signal group</button>}{canManageTelegram && <button className="button button--quiet" type="button" onClick={() => navigate('telegram')}>Telegram readers</button>}</div></div>
          <div className="overview-grid" aria-label="Workspace status"><article className="overview-card"><span className="status-label">Session</span><strong>Securely signed in</strong><small>Your current Super Signals session is active.</small></article><article className="overview-card"><span className="status-label">Role</span><strong>{account.role_label}</strong><small>{account.permissions.length} approved permissions.</small></article><article className="overview-card"><span className="status-label">Trading state</span><strong>Live trading disabled</strong><small>Source operating states are separate from trade execution.</small></article></div>
