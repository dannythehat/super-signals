...
export function Mt5DemoConnectionPanel({ apiBaseUrl }: { apiBaseUrl: string }) {
  ...
  async function connect(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(`${apiBaseUrl}/owner/mt5/demo/connect`, {
        ...
        body: JSON.stringify({ metaapi_token: form.get('metaapi_token'), login: form.get('login'), password: form.get('password'), server: form.get('server') }),
      });
      ...
      setMessage((body as Connection).status === 'connected' ? 'Vantage MT5 demo connected.' : 'Connection submitted. MetaAPI is still connecting.');
  ...
  return <section aria-labelledby="mt5-demo-title">
    <div className="overview-hero">
      ...
      <p className="intro">Connect the owner's demo account through MetaAPI. This screen cannot place trades. Your MT5 password is submitted only for the MetaAPI connection and is not stored by Super Signals.</p>
    </div>
    ...
    {!connection?.configured && <form className="auth-form" onSubmit={connect} autoComplete="off">
      ...
      <label>Vantage MT5 server<input name="server" required autoComplete="off" placeholder="Exact server name shown in MT5" /></label>
