...
const flowId = '937a1a41-74f2-4cab-9016-16d50cbc7e78';
...
const codeAuthorization = {
  ...
  phone_hint: '+35***567',
  expires_at: '2026-08-07T04:05:00Z',
};
...
function fillSamePhoneStartForm() {
...
describe('TelegramConnectionPanel', () => {
  ...
  it('connects with a same-phone Telegram login code without exposing the code', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse(200, []))
      .mockResolvedValueOnce(jsonResponse(201, codeAuthorization))
    ...
    fillSamePhoneStartForm();
    fireEvent.click(screen.getByRole('button', { name: 'Send Telegram code' }));

    expect(await screen.findByText('Enter the Telegram code')).toBeInTheDocument();
    expect(screen.getByText('+35***567')).toBeInTheDocument();
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      '/api/admin/telegram/accounts/authorize/code',
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      3,
      `/api/admin/telegram/accounts/authorize/${flowId}/code`,
      expect.objectContaining({
        method: 'POST',
        credentials: 'include',
  ...
  it('completes same-phone Telegram two-step verification without exposing the password', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse(200, []))
      .mockResolvedValueOnce(jsonResponse(201, codeAuthorization))
    ...
    const codeInput = await screen.findByLabelText('Telegram login code');
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      4,
      `/api/admin/telegram/accounts/authorize/${flowId}/password`,
      expect.objectContaining({
        method: 'POST',
        credentials: 'include',
  ...
  it('keeps QR login available as an optional second-device method', async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValueOnce(jsonResponse(200, []))
    ...
    openTelegramManager();
    await screen.findByText('No Telegram reader account is connected yet.');
    fireEvent.click(screen.getByRole('button', { name: 'Use QR on a second device instead' }));
    fireEvent.change(screen.getByLabelText('Private account label'), {
      target: { value: 'Primary signal reader' },
    });
