...
from app.ai_message_supervisor import AiMessageDecision, AiSupervisorError, OpenAiMessageSupervisor
...
class AiMessagePipeline:
    ...
    def process_latest_revision(self, source_id: UUID, telegram_message_id: int) -> AiPipelineResult:
        ...
        return self._process_revision(source_id, telegram_message_id, revision_index=int(revision_index))
    ...
    def _process_revision(
        ...
        *,
        ...
        with self._session_factory() as session:
            ...
            if existing_signal_id is not None:
                execution_started = bool(
                    session.execute(
                        text("SELECT EXISTS(SELECT 1 FROM positions WHERE signal_id = :signal_id)"),
        ...
        if revision_index > 0 and previous_text is not None and raw_text == previous_text:
            decision = self._non_actionable_without_ai(raw_text, reason="identical_text_revision")
            self._store_decision(row["message_id"], revision_index, decision)
            return AiPipelineResult(
                True, decision.decision, decision.action, existing_signal_id, None, decision.source, decision.reason
            )

        if not raw_text.strip():
            payload = row["raw_payload"] if isinstance(row["raw_payload"], dict) else {}
            reason = "media_only_requires_media_ingestion" if bool(payload.get("has_media")) else "empty_message"
        ...
        if revision_index > 0 and existing_signal_id is not None:
            if execution_started:
                decision = replace(decision, decision="non_actionable", action="skip", reason="post_execution_edit")
            else:
                decision = replace(decision, decision="new_trade", action="execute")
        ...
        if revision_index > 0 and existing_signal_id is not None:
            if execution_started:
                ...
                revision_result = self._signals.apply_pre_execution_revision(
                    ...
                    revision_index=revision_index,
                    execute=(decision.decision == "new_trade" and decision.action == "execute"),
    ...
    def _store_decision(self, message_id: UUID, revision_index: int, decision: AiMessageDecision) -> None:
        with self._session_factory() as session:
            session.execute(
                text(
                    """
                    INSERT INTO ai_message_decisions (
                        message_id,revision_index,decision,action,confidence,reason,
                        extracted,model,response_id,latency_ms,decision_source,raw_text_sha256
                    ) VALUES (
                        :message_id,:revision_index,:decision,:action,:confidence,:reason,
                        CAST(:extracted AS jsonb),:model,:response_id,:latency_ms,:decision_source,:raw_text_sha256
                    )
    ...
    def _previous_text(session: Session, message_id: UUID, revision_index: int) -> str | None:
        ...
        return session.execute(
            text(
                """
                SELECT raw_text FROM message_revisions
                WHERE message_id = :message_id
                  AND revision_index = :revision_index
                """
            ),
