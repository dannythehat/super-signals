...
class PushDeliveryResult:
    seeded:int;sent:int;failed:int;suppressed:int;broker_trade_action_created:bool=False
class Day34PushNotificationManager:
    def __init__(self,*,session_factory:sessionmaker[Session],vapid_private_key:str,vapid_subject:str,poll_seconds:int=3)->None:
        ...
        self._session_factory=session_factory;self._vapid_private_key=vapid_private_key;self._vapid_subject=vapid_subject;self._poll_seconds=poll_seconds;self._task:asyncio.Task[None]|None=None;self._stop_event=asyncio.Event()
    ...
    def _send_claimed(self,r:dict[str,Any])->str:
        ...
        try:webpush(subscription_info=sub,data=data,vapid_private_key=self._vapid_private_key,vapid_claims={'sub':self._vapid_subject},ttl=300,timeout=10)
            ...
            if code in{404,410}:self._suppress_expired(did,sid,code);return'suppressed'
            self._record_failure(did,sid,code=f"web_push_{code or 'error'}",reason=str(e)[:500]);return'failed'
    ...
    def _record_success(self,did:UUID,sid:UUID)->None:
    ...
    def _record_failure(self,did:UUID,sid:UUID,*,code:str,reason:str)->None:
    ...
    def _suppress_expired(self,did:UUID,sid:UUID,code:int)->None:
        with self._session_factory() as s:s.execute(text("UPDATE push_notification_deliveries SET status='suppressed',failure_code=:c,failure_reason='Browser push subscription expired or was removed.',updated_at=now() WHERE id=:id"),{'id':did,'c':f'web_push_{code}'});s.execute(text("UPDATE push_subscriptions SET enabled=false,failure_count=failure_count+1,last_failure_at=now(),updated_at=now() WHERE id=:id"),{'id':sid});s.commit()
