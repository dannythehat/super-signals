    ...
    def _recover_stale_sending(self) -> int:
        with self._session_factory() as session:
            rows = session.execute(text("""
                ...
                SET status='failed', failure_code='push_delivery_recovered_after_restart',
                    failure_reason='Recovered stale sending state after worker restart.', updated_at=now()
                WHERE status='sending' AND sent_at IS NULL AND attempted_at IS NOT NULL
                  AND attempted_at < now() - make_interval(secs => :stale_seconds)
                RETURNING id
            """), {"stale_seconds": _STALE_SENDING_SECONDS}).all()
            session.commit()
            return len(rows)
        ...
        with self._session_factory() as session:
            session.execute(text("""
                ...
                SELECT
                    ...
                    NULL, NULL, ps.user_id, 'user', 'push_test',
                ...
                WHERE ps.enabled=true
                  AND NOT EXISTS (
                    SELECT 1 FROM notification_events AS n
                    WHERE n.event_key='push-enabled:' || ps.id::text || ':' ||
                        to_char(ps.active_since AT TIME ZONE 'UTC', 'YYYYMMDDHH24MISSUS')
                  )
                ON CONFLICT (event_key) DO NOTHING
            """))
            rows = session.execute(text("""
                INSERT INTO push_notification_deliveries (notification_id, subscription_id, status)
                SELECT n.id, ps.id, 'pending'
                FROM notification_events AS n
                JOIN push_subscriptions AS ps
                  ON ps.enabled=true AND n.created_at >= ps.active_since
                 AND (n.audience='shared' OR (n.audience='user' AND n.user_id=ps.user_id))
                WHERE NOT EXISTS (
                    SELECT 1 FROM push_notification_deliveries AS d
                    WHERE d.notification_id=n.id AND d.subscription_id=ps.id
                )
    ...
    def _claim_one(self) -> dict[str, Any] | None:
        with self._session_factory() as session:
            row = session.execute(text("""
                WITH candidate AS (
                    SELECT d.id FROM push_notification_deliveries AS d
                    JOIN push_subscriptions AS ps ON ps.id=d.subscription_id
                    WHERE d.status IN ('pending','failed') AND d.attempt_count < :max_attempts AND ps.enabled=true
                    ORDER BY d.created_at, d.id FOR UPDATE OF d SKIP LOCKED LIMIT 1
                )
                UPDATE push_notification_deliveries AS d
                SET status='sending', attempt_count=d.attempt_count+1, attempted_at=now(),
                    failure_code=NULL, failure_reason=NULL, updated_at=now()
                FROM candidate AS c WHERE d.id=c.id RETURNING d.id
            """), {"max_attempts": _MAX_ATTEMPTS}).mappings().first()
            if row is None:
                session.commit()
                return None
            payload = session.execute(text("""
                SELECT d.id AS delivery_id, d.subscription_id, n.id AS notification_id,
                       n.kind, n.title, n.body, ps.endpoint, ps.p256dh, ps.auth
                FROM push_notification_deliveries AS d
                JOIN notification_events AS n ON n.id=d.notification_id
                JOIN push_subscriptions AS ps ON ps.id=d.subscription_id
                WHERE d.id=:delivery_id
            """), {"delivery_id": row["id"]}).mappings().one()
            session.commit()
            return dict(payload)

    def _send_claimed(self, row: dict[str, Any]) -> str:
        delivery_id = UUID(str(row["delivery_id"])); subscription_id = UUID(str(row["subscription_id"]))
        data = json.dumps({
            "notification_id": str(row["notification_id"]), "kind": str(row["kind"]),
            "title": str(row["title"]), "body": str(row["body"]), "url": "/",
            "private_account_data_included": False,
        }, separators=(",", ":"))
        subscription = {"endpoint": str(row["endpoint"]), "keys": {"p256dh": str(row["p256dh"]), "auth": str(row["auth"])}}
        try:
            webpush(subscription_info=subscription, data=data, vapid_private_key=self._vapid_private_key,
                    vapid_claims={"sub": self._vapid_subject}, ttl=300, timeout=10)
        except WebPushException as exc:
            status_code = getattr(getattr(exc, "response", None), "status_code", None)
            if status_code in {404, 410}:
                self._suppress_expired(delivery_id, subscription_id, status_code); return "suppressed"
            ...
            self._record_failure(delivery_id, subscription_id, code="web_push_unexpected", reason=str(exc)[:500]); return "failed"
