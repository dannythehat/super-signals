    ...
    async def fetch_context(self, *, as_of: datetime) -> AidyCanonicalContext:
        ...
        if gold_state:
            ...
            if contract_version == "aidy_provider_gold_state_v2":
                ...
                gold_digest = str(gold_state.get("gold_state_digest") or "")
                if len(gold_digest) != 64 or any(
                    ch not in "0123456789abcdef" for ch in gold_digest
                ):
                ...
                movement = gold_state.get("movement_investigation")
                if movement is not None:
                    if not isinstance(movement, dict):
                        raise TypeError("aidy_context_gold_movement_investigation_invalid")
                    if (
                        movement.get("investigator_version")
                        != "aidy_gold_movement_investigator_v1"
                    ):
                        raise ValueError("aidy_context_gold_movement_version_invalid")
                    movement_at = _utc_strict(
                        movement.get("as_of_utc"),
                        field="gold_movement_as_of_utc",
                    )
                    if movement_at != context_at or movement_at > requested:
                        raise ValueError("aidy_context_gold_movement_time_mismatch")
                    if movement.get("research_only") is not True:
                        raise ValueError("aidy_context_gold_movement_research_only_required")
                    if movement.get("future_values_used") is not False:
                        raise ValueError("aidy_context_gold_movement_future_values_forbidden")
                    if movement.get("live_money_execution_allowed") is not False:
                        raise ValueError("aidy_context_gold_movement_execution_forbidden")
                    if movement.get("predictive_edge_claimed") is not False:
                        raise ValueError("aidy_context_gold_movement_predictive_claim_forbidden")
                    movement_digest = str(movement.get("investigation_digest") or "")
                    if len(movement_digest) != 64 or any(
