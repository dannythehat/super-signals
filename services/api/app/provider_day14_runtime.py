"""Production runtime for Provider Intelligence Day 14 governance.

The runtime evaluates the 40 discovery providers against their own immutable Day 13
preregistration boundary and the existing provider_research_profiles state machine:
learning -> shadow -> qualified. `qualified` means paper-research eligibility only.

Promotion/demotion thresholds are a builder recommendation until explicitly owner-approved.
No code in this module changes sources.status, broker routing, member risk, position sizing,
or live-money authority. A human gate remains mandatory after qualified.
"""
...
EXPECTED_HYPOTHESES_PER_PROVIDER = (
    len(day13.SIDES)
    * len(day13.SESSIONS)
    * len(day13.DURATION_BUCKETS)
    * sum(len(values) for values in day13.REGIME_VALUES.values())
)
...
def classify_performance(
    ...
    *,
    ...
    if len(values) < minimum_oos_n:
        return "INSUFFICIENT_OOS", fmean(values) if values else None, None, None
    if tested_fingerprint_cells < minimum_tested_fingerprint_cells:
        return "FINGERPRINT_NOT_READY", fmean(values), None, None
...
def decide_governance(
    *,
    ...
    """Deterministic evidence policy for the existing research state machine only."""
    ...
    if evidence_state in {"INSUFFICIENT_OOS", "FINGERPRINT_NOT_READY", "UNCERTAIN"}:
        return GovernanceDecision(
            ...
            "forward evidence does not clear the minimum-N/fingerprint gate",
        )
    ...
    if evidence_state == "POSITIVE_CONFIDENT":
        if current_research_state == "learning":
            return GovernanceDecision(
                "PROMOTE", "shadow", False, "positive OOS R plus tested conditional fingerprint"
            )
        ...
        if current_research_state == "qualified":
            return GovernanceDecision(
                ...
                "qualified",
                True,
                "qualified is terminal for automation; human live gate remains mandatory",
            )
        return GovernanceDecision(
            "RETEST",
            current_research_state,
...
def simulated_governance_acceptance() -> dict[str, Any]:
    """Synthetic state-machine proof only; never provider evidence."""
    learning_to_shadow = decide_governance(
    ...
    shadow_to_qualified = decide_governance(
    ...
    first_adverse = decide_governance(
        current_research_state="qualified",
        policy_approval_status=OWNER_APPROVED,
    ...
    sustained_adverse = decide_governance(
        current_research_state="qualified",
        policy_approval_status=OWNER_APPROVED,
        evidence_state="NEGATIVE_CONFIDENT",
    ...
    small_n = decide_governance(
        current_research_state="learning",
        policy_approval_status=OWNER_APPROVED,
        evidence_state="INSUFFICIENT_OOS",
        sustained_decay=False,
        duplicate_review=False,
        fresh_evidence_since_transition=True,
    )
    passed = bool(
        learning_to_shadow.proposed_research_state == "shadow"
        and shadow_to_qualified.proposed_research_state == "qualified"
        and shadow_to_qualified.paper_qualified
        and first_adverse.proposed_action == "RETEST"
        and sustained_adverse.proposed_research_state == "shadow"
        and unapproved.proposed_action == "HOLD"
        and replayed.proposed_action == "HOLD"
        and small_n.proposed_action == "HOLD"
    )
    return {
        "synthetic_only": True,
        "learning_to_shadow": asdict(learning_to_shadow),
        "shadow_to_qualified": asdict(shadow_to_qualified),
        "first_adverse_snapshot": asdict(first_adverse),
        "sustained_decay": asdict(sustained_adverse),
        "unapproved_policy": asdict(unapproved),
        "same_evidence_replay": asdict(replayed),
        "small_n": asdict(small_n),
...
def _preregistration_boundaries(session: Any) -> dict[UUID, datetime]:
    rows = session.execute(
        text(
            """
            SELECT source_id,MIN(preregistered_at) AS first_boundary,
                   MAX(preregistered_at) AS last_boundary,COUNT(*) AS hypotheses
    ...
    for row in rows:
        if row["first_boundary"] != row["last_boundary"]:
            raise RuntimeError("provider_preregistration_boundary_not_frozen")
        if int(row["hypotheses"] or 0) != EXPECTED_HYPOTHESES_PER_PROVIDER:
            raise RuntimeError("provider_preregistration_hypothesis_count_mismatch")
...
def _provider_evidence(
    *,
    ...
    source_observations = [
        ...
        if observation.source_id == source_id and observation.signal_posted_at > preregistered_at
    ]
    realized_by_trade = {observation.trade_id: observation.realized_r for observation in source_observations}
    values = [float(realized_by_trade[key]) for key in sorted(realized_by_trade, key=str)]
    ...
    positive = sum(
        bool(row["builder_gate_candidate"])
        and row["shrunken_effect_r"] is not None
        and float(row["shrunken_effect_r"]) > 0
    ...
    payload = {
        ...
        "trade_ids": [str(key) for key in sorted(realized_by_trade, key=str)],
        "r_values": [round(value, 8) for value in values],
        "tested_fingerprint_cells": tested,
...
def run() -> dict[str, Any]:
    ...
    try:
        with session_factory() as session:
            ...
            # New discovery providers get a fresh append-only preregistration timestamp before
            # any of their future outcomes can enter governance evidence. Existing providers are
            # untouched because Day 13's registry insert is conflict-safe and immutable.
            day13._ensure_preregistry(session, preregistered_at=cutoff)
            session.commit()

            profiles = _shadow_profiles(session)
            boundaries = _preregistration_boundaries(session)
            profile_ids = {UUID(str(row["source_id"])) for row in profiles}
            if not profile_ids.issubset(boundaries):
                raise RuntimeError("shadow_provider_missing_preregistration_boundary")
            ...
            for profile in profiles:
                ...
                evidence_by_source[source_id] = _provider_evidence(
                    ...
                    preregistered_at=boundaries[source_id],
                    policy=policy,
                )
                ...
                "providers": [
                    {
                        ...
                        "preregistered_at": boundaries[source_id].isoformat(),
            ...
            engineering_status = (
                "ENGINEERING_PROVEN" if simulation["acceptance_passed"] else "FAILED_SIMULATION_ACCEPTANCE"
            )
                            ...
                            "reason": json.dumps(
                                {
                                    "reason": row["reason"],
                                    "policy_approval_status": str(policy["approval_status"]),
                                    "preregistered_at": boundaries[row["source_id"]].isoformat(),
                                },
                                sort_keys=True,
                            ),
                        }
            ...
            boundary_values = list(boundaries[source_id] for source_id in profile_ids)
            summary = {
                "run_id": str(run_id),
                ...
                "provider_preregistration_boundary_count": len(set(boundary_values)),
                "oldest_preregistration_boundary": min(boundary_values).isoformat() if boundary_values else None,
                "newest_preregistration_boundary": max(boundary_values).isoformat() if boundary_values else None,
