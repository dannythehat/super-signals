...
async def replay_signal_calibration(
    *, session: Any, client: AidyMarketClient, signal: dict[str, Any], now: datetime
    ...
    """Reconstruct paper independently, then compare exact keyed legs to broker truth."""
    ...
    try:
        ...
        return {
            ...
            "replay_to": replay_to,
            "broker_deal_count": 0,
            "exclusion_reason": f"paper_allocation_invalid:{exc}"[:160],
        }

    window = await client.fetch_calibration_m1(
        window_id=str(signal["id"]), start=replay_from, end=replay_to
    )
    if not window.complete:
        return {
            ...
            "broker_deal_count": 0,
            "exclusion_reason": (
                "calibration_m1_incomplete:" + window.missing_open_times[0].isoformat()
            )[:160],
        }
    ...
    sibling_entries = [
        (int(entry.entry_index), (entry.low + entry.high) / Decimal("2"))
    ...
    for entry in entries:
        ...
        leg_ids = {
            item.tp_index: _leg_id(signal["id"], entry_index, item.tp_index)
    ...
    if any(state.score_block_reason is not None for state in states):
        blocked = next(
            state.score_block_reason for state in states if state.score_block_reason
        )
        return {
            "replay_from": replay_from,
            "replay_to": replay_to,
        ...
        for leg in state.legs:
            ...
            if key in paper_legs:
                return {
                    ...
                    "replay_to": replay_to,
                    "broker_deal_count": 0,
                    "exclusion_reason": "paper_leg_key_duplicate",
                }
            paper_legs[key] = leg.realized_r

    # Broker truth is deliberately read only after the paper side is fully reconstructed.
    # It is never used to choose paper entries, targets, ordering, exits, or lifecycle.
    ...
    if broker_error is not None or broker_legs is None or broker_lifecycle is None:
        return {
            ...
            "exclusion_reason": broker_error or "broker_leg_truth_unavailable",
        }
    ...
    if set(paper_legs) != set(broker_legs):
        return {
            ...
            "replay_to": replay_to,
            "broker_deal_count": broker_deals,
            "exclusion_reason": "paper_broker_leg_key_mismatch",
        }

    leg_deltas = {
        key: abs(paper_legs[key] - broker_legs[key]) for key in paper_legs
    }
