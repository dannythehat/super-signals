"""Canonical provider management shared by paper and future LIVE.

This module owns layer-aware target selection, event-time safety, profit-qualified close,
provider-price mapping, add-market handling and broker-authoritative protection. Broker
mutations always use exact mapped position/order IDs.

Paper and future LIVE use this exact management engine. The member adapter below changes
only approved-account selection; ordinary member LIVE mutation remains disabled at the
outer distribution switch until explicitly enabled.
"""
...
from app.mt5_management_day27 import Day27ManagementError, Day27ManagementResult, _Account
...
class MemberTradingManagementService(CanonicalTradingManagementService):
    """Run canonical management against one approved member LIVE account."""
