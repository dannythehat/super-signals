...
Revision ID: 0034_reviewed_provider_trade_result
...
revision: str = "0034_reviewed_provider_trade_result"
...
def upgrade() -> None:
    # Reviewed outcomes must survive the normal broker-ledger rebuild. Broker evidence
    # remains immutable; these three rows are explicitly marked as provider-reviewed.
    op.execute(
        """
    ...
    # The provider message reported TP1 + TP2 secured at double lot size and the
    # product owner has confirmed that TP3 also won. Record the three legs using the
    # provider's declared 4341 entry and 0.02 lot double-size representation.
    op.execute(
        f"""
        UPDATE positions
        SET status='closed',
    ...
    # Keep the materialised summary cards/charts aligned with the corrected ledger.
    op.execute(
        f"""
