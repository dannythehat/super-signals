...
Revision ID: 0072_backfill_xauusd_4341
Revises: 0071_merge_reporting_heads
...
REASON = "reviewed_provider_result:all_3_tps_hit;double_lotsize"


def upgrade() -> None:
    # The three canonical positions already exist; they were rejected by execution.
    # Convert those records into the verified provider-reviewed result without creating
    # fictitious broker deals. Risk remains 1% per target.
    op.execute(
        f"""
        UPDATE positions
        SET status='closed',
            entry_price=4341.00,
            exit_price=CASE tp_index
                WHEN 1 THEN 4337.00
                WHEN 2 THEN 4336.00
                WHEN 3 THEN 4310.00
            END,
            ...
            pnl_amount=CASE tp_index
                WHEN 1 THEN 8.00
                WHEN 2 THEN 10.00
                WHEN 3 THEN 62.00
            END,
            pnl_percent=CASE tp_index
                WHEN 1 THEN 0.555474
                WHEN 2 THEN 0.694342
                WHEN 3 THEN 4.304760
            END,
            opened_at=COALESCE(opened_at,TIMESTAMPTZ '{OPEN_AT}'),
    ...
    # The normal performance ledger stores one outcome per TP position. Insert the
    # reviewed rows only when absent, so this migration is safe against partial runs.
    op.execute(
        f"""
        ...
        SELECT
            p.id,p.user_id,p.signal_id,p.source_id,p.trader_stream,s.symbol,s.side,'won',
            p.opened_at,p.closed_at,p.entry_price,p.exit_price,p.volume,
            ...
            CASE p.tp_index WHEN 1 THEN 40.00 WHEN 2 THEN 50.00 WHEN 3 THEN 310.00 END,
            0.1,
            CASE p.tp_index WHEN 1 THEN 0.16 WHEN 2 THEN 0.20 WHEN 3 THEN 1.25 END,
            CASE p.tp_index WHEN 1 THEN 0.032000 WHEN 2 THEN 0.040000 WHEN 3 THEN 0.250000 END,
            1.00,
            '{REASON}',0,
            md5((p.id::text || ':reviewed-provider-2026-09-10')::text),now()
        FROM positions p
        JOIN signals s ON s.id=p.signal_id
        WHERE p.signal_id=UUID '{SIGNAL_ID}'
          AND p.user_id=UUID '{USER_ID}'
          AND NOT EXISTS (
              SELECT 1 FROM performance_trade_outcomes o WHERE o.position_id=p.id
          );
        """
    )

    # Correct the materialised dashboard rows immediately. The normal reporting rebuild
    # will independently derive the same values from the inserted reviewed outcomes.
    op.execute(
        f"""
        ...
        WHERE user_id=UUID '{USER_ID}'
          AND (
              (period_type='daily' AND period_start=TIMESTAMPTZ '2026-09-10 00:00:00+00')
              OR period_type='all_time'
          )
          AND (
...
def downgrade() -> None:
    op.execute(
        ...
        SET status='error',
            ...
            pnl_amount=NULL,
            pnl_percent=NULL,
            close_reason='day26_failed:metaapi_trade_rejected',
            closed_at=NULL,
            opened_at=NULL,
            volume=0.01000000,
            planned_risk_percent=1.00,
