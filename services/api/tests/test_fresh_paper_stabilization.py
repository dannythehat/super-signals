...
def test_tig_two_entries_four_targets_use_four_atomic_positions_not_eight() -> None:
    ...
    plan = PaperFreshStartExecutionService._allocation_pairs(entries, targets)
    assert len(plan) == 4
...
@pytest.mark.asyncio
async def test_pending_gateway_resolves_day28_style_wrapper_to_real_transport() -> None:
