"""Canonical ordinary-member distribution and isolation acceptance."""
...
class _Trade:
    ...
    async def place_market_order(self, **kwargs: object) -> MetaApiMarketOrderResult:
        ...
        return MetaApiMarketOrderResult(
            ...
            position_id=f"position-{len(self.calls)}",
            numeric_code=10009,
            string_code="TRADE_RETCODE_DONE",
        )

    async def close_position(self, **_: object) -> None:
        return None
...
class _ExecutionHarness(MemberTradingExecutionService):
    def __init__(self) -> None:
        ...
        super().__init__(
            ...
            cipher=None,
            read_gateway=self.read,
            margin_gateway=self.margin,
            trade_gateway=self.trade,
    ...
    def _provider_zone(self, signal_id):
        return Decimal("4000"), Decimal("4000")
    ...
    def _decrypt_token(self, account):
    ...
    def _audit_blocked(self, **kwargs):
        return None

    def _record_execution_failure(self, **kwargs):
        return None
...
class _DistributionHarness(MemberDistributionService):
    ...
    def _targets(self):
        return (
            MemberDistributionTarget(U1, Decimal("0.5"), False),
            MemberDistributionTarget(U2, Decimal("2"), True),
            MemberDistributionTarget(U3, Decimal("1"), False),
        )
...
def test_member_live_switch_is_off_by_default(monkeypatch) -> None:
    ...
    assert {item.error_code for item in result.outcomes} == {"live_execution_disabled"}
    assert execution.trade.calls == []
...
class _NoManagement:
    async def distribute(self, **kwargs):
        return SimpleNamespace(any_management_succeeded=False)
...
class _RouterHarness(CanonicalExecutionDispatcher):
    ...
    def _audit_new_trade_route(self, **kwargs):
        self.audits.append(("route", kwargs))
...
def test_owner_demo_failure_does_not_block_eligible_member_execution() -> None:
    ...
    success_payload = next(
        item[1]["payload"] for item in router.audits if item[0] == "success"
    )
...
def test_one_member_management_failure_does_not_block_another_member(monkeypatch) -> None:
    monkeypatch.setenv("SUPER_SIGNALS_LIVE_EXECUTION_ENABLED", "1")
    service = _ManagementHarness()
    result = asyncio.run(
        service.distribute(signal_id=SIGNAL, lifecycle_event_id=uuid4())
    )
