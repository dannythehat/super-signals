"""Regressions from the 19 August paper-trading audit.

These tests lock the failure modes found in real provider traffic: valid directional
geometry must stay executable, an earlier deterministic validation failure must not
poison a later corrected provider revision, ambiguous MetaAPI mutation failures must not
be blindly retried, and vanished pending tickets must be resolved from broker history
rather than guessed from `/orders` absence.
"""
...
def test_pending_history_cancelled_ticket_is_exact_broker_truth() -> None:
    ...
    matched = PaperPendingReconciler._matching_history_order(local, history)
...
def test_pending_history_wrong_client_id_is_not_allowed_to_close_local_truth() -> None:
    ...
    with pytest.raises(ValueError, match="pending_history_client_id_mismatch"):
        PaperPendingReconciler._matching_history_order(local, history)
...
def test_missing_pending_history_remains_unresolved_not_assumed_cancelled() -> None:
    ...
    assert PaperPendingReconciler._matching_history_order(local, []) is None
